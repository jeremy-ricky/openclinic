...
<%
	String accessright="academy.chocolateallergy"; // clet unique de la page
%>
...
<form name="transactionForm" id="transactionForm" method="POST" action='<c:url value="/healthrecord/updateTransaction.do"/>?ts=<%=getTs()%>'>
    ...
    <table class="list" width="100%" cellspacing="1" cellpadding='0'>
        ...
        <%-- DEBUT LES AJOUTS ICI AVEC LE NOUVEAU FORMULAIRE --%>
        <tr>
        	<td>
        		<table width="100%">
        			<tr>
						<td class='admin'><%=getTran(request, "web","chocolatetype",sWebLanguage) %></td>
			        	<td class='admin2'><%=SH.writeDefaultCheckBoxes((TransactionVO)transaction, request, "chocolatetypes", "ITEM_TYPE_CHOCOLATETYPE", sWebLanguage, true) %></td>
			        	<td class='admin'><%=getTran(request, "web","chocolatequantity",sWebLanguage) %></td>
			        	<td class='admin2'><%=SH.writeDefaultNumericInput(session, (TransactionVO)transaction, "ITEM_TYPE_CHOCOLATEQUANTITY", 6, sWebLanguage, "")%></td>
			        </tr>
					<tr>
						<td class='admin'><%=getTran(request,"web","dateofmanufacture",sWebLanguage)%>&nbsp;</td>
						<td class='admin2'><%=SH.writeDefaultDateInput(session, (TransactionVO)transaction, "ITEM_TYPE_DATEOFMANUFACTURE", sWebLanguage, sCONTEXTPATH) %></td>
						<td class='admin'><%=getTran(request,"web","expirationdate",sWebLanguage)%>&nbsp;</td>
						<td class='admin2'><%=SH.writeDefaultDateInput(session, (TransactionVO)transaction, "ITEM_TYPE_EXPIRATIONDATE", sWebLanguage, sCONTEXTPATH) %></td>    
					</tr>
					<tr>
						<td class='admin'><%=getTran(request, "web","chocolatesymptoms",sWebLanguage) %></td>
			        	<td class='admin2'><%=SH.writeDefaultCheckBoxes((TransactionVO)transaction, request, "chocolatesymptoms", "ITEM_TYPE_CHOCOLATESYMPTOMS", sWebLanguage, true) %></td>
			        	<td class='admin'><%=getTran(request, "web","weightinkg",sWebLanguage) %></td>
			        	<td class='admin2'><%=SH.writeDefaultNumericInput(session, (TransactionVO)transaction, "ITEM_TYPE_WEIGHTINKGS", 6, sWebLanguage, "")%></td>
			        </tr>
        		</table>
        	</td>
        </tr>
        <%-- FIN LES AJOUTS ICI AVEC LE NOUVEAU FORMULAIRE --%>
    </table>
